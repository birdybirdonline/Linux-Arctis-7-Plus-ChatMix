...
class ArctisChatMix:
    ...
    def _init_VAC(self):
        ...
        try:
            destroy_arctis_game = os.system("pw-cli destroy Arctis_Game 2>/dev/null")
            destroy_arctis_chat = os.system("pw-cli destroy Arctis_Chat 2>/dev/null")
        ...
        try:
            ...
            os.system(f"""pw-cli create-node adapter '{{
                ...
                node.description="{self.udev_device.name} Game"
            ...
            os.system(f"""pw-cli create-node adapter '{{
                ...
                node.description="{self.udev_device.name} Chat"
                media.class=Audio/Sink
                monitor.channel-volumes=true
                object.linger=true
                audio.position=[{' '.join(self.udev_device.audio_position)}]
    ...
    def start_modulator_signal(self):
        ...
        self.log.info("-"*45)
        self.log.info("Arctis ChatMix Enabled!")
        self.log.info("-"*45)
    ...
    def die_gracefully(self, sink_creation_fail=False, trigger=None):
        ...
        if trigger is not None:
            ...
            self.log.info("-"*45)
            self.log.info("Artcis ChatMix shut down gracefully... Bye Bye!")
            self.log.info("-"*45)
            sys.exit(0)
...
if __name__ == '__main__':
    arctis_pcm_service = ArctisChatMix()
